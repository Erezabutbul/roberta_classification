...
res_df = pd.read_csv(
    os.path.join(run_dir, 'train\\data\\data_full.csv'),
...
for train_index, val_index in skf.split(res_df["text"], res_df["labels"]):
    ...
    train_df.to_csv(
        os.path.join(run_dir, 'train\\data\\train_balanced.csv'),
        index=True, index_label="index", encoding="utf_8_sig")
    df = train_df
    test_df.to_csv(
        os.path.join(run_dir, 'train\\data\\test_balanced.csv'),
        index=True, index_label="index", encoding="utf_8_sig")
    break
...
for train_index, val_index in skf.split(df["text"], df["labels"]):
    ...
    train_df_tmp.to_csv(
        os.path.join(run_dir, 'train\\data\\splits', train_str), index=True, index_label="index", encoding="utf_8_sig")
    eval_df_tmp.to_csv(
        os.path.join(run_dir, 'train\\data\\splits', eval_str), index=True, index_label="index", encoding="utf_8_sig")
